...
import { type StorageResult, StorageResults } from './types/storage-result';
...
export class CredentialStoreStorage extends BaseSecureStorage {
    ...
    async store(key: string, value: Buffer, options: StorageOptions = {}): Promise<StorageResult<void>> {
        try {
            ...
            if (!this.isAvailable) {
                return this.createResult<void>(false, undefined, 'Credential store not available');
            }
            ...
            return this.createResult<void>(true);
        } catch (error) {
        ...
        try {
            ...
            if (StorageResults.isOk(metadataResult) && metadataResult.value?.chunks) {
                ...
                for (let i = 0; i < numChunks; i++) {
                    ...
                    if (!chunkData) {
                        return this.createResult<Buffer>(false, undefined, `Missing chunk ${i} for key ${key}`);
                    }
    ...
    async remove(key: string): Promise<StorageResult<void>> {
        try {
            ...
            return this.handleError<void>(error, 'remove');
        }
    }
    ...
    async clear(): Promise<StorageResult<void>> {
        try {
            ...
            return this.handleError<void>(error, 'clear');
        }
    }
    ...
    async test(): Promise<StorageResult<void>> {
        try {
            ...
            return this.createResult<void>(false, undefined, 'Test failed');
        } catch (error) {
            return this.handleError<void>(error, 'test');
        }
    }
    ...
    private async getStoredMetadata(key: string): Promise<StorageResult<StorageMetadata>> {
        try {
            ...
            return this.createResult<StorageMetadata>(false, undefined, 'No metadata found');
        } catch (error) {
            return this.createResult<StorageMetadata>(false, undefined, `Failed to get metadata: ${error}`);
        }
    }
