...
export class StorageMigrator {
        ...
        try {
            ...
            for (const key of sourceKeys) {
                try {
                    ...
                    if (!StorageResults.isOk(targetExists) || !targetExists.value) {
                    ...
                    if (plan.validation === ValidationStrategy.DataIntegrity ||
                        ...
                        if (!StorageResults.isOk(sourceData) || !StorageResults.isOk(targetData)) {
                            statistics.corruptedKeys++;
                            errors.push(`Cannot read data for key: ${key}`);
                            continue;
                        }

                        if (!sourceData.value.equals(targetData.value)) {
                            statistics.corruptedKeys++;
    ...
    private async executeCopyThenValidate(
        ...
        for (const key of keys) {
            ...
            try {
                const sourceData = await plan.source.retrieve(key);
                if (!StorageResults.isOk(sourceData)) {
                    this.addError(progress, {
                        key,
                        operation: 'read',
                        error: StorageResults.isError(sourceData) ? sourceData.error.message || 'Failed to read from source' : 'Failed to read from source',
                        timestamp: new Date(),
                        recoverable: true,
                    });
                    continue;
                ...
                const targetResult = await plan.target.store(key, sourceData.value);
                if (!StorageResults.isOk(targetResult)) {
                    this.addError(progress, {
                        key,
                        operation: 'write',
                        error: StorageResults.isError(targetResult) ? targetResult.error.message || 'Failed to write to target' : 'Failed to write to target',
                        timestamp: new Date(),
                        recoverable: true,
                    });
                    continue;
        ...
        for (const key of keys) {
            ...
            try {
                ...
                if (plan.validation === ValidationStrategy.DataIntegrity ||
                    ...
                    if (!StorageResults.isOk(verifyResult) || !verifyResult.value.equals(sourceData.value)) {
                        this.addError(progress, {
                            key,
                            operation: 'validate',
    ...
    private async getSourceKeys(storage: SecureStorage): Promise<string[]> {
        ...
        if (!StorageResults.isOk(listResult)) {
            const errorMessage = StorageResults.isError(listResult) ? listResult.error.message || 'Unknown error' : 'Failed to list keys';
