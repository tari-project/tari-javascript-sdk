...
class ExchangeDepositManager {
  ...
  recordDeposit(address, amount, txId) {
    ...
    console.log(
      `💰 DEPOSIT: User ${deposit.userId} received ${formatBalance(amount)} (Total: ${formatBalance(deposit.totalReceived)})`
    );
    return true;
  }
}
    ...
    async (walletHandle) => {
      ...
      const monitorTransactions = async () => {
        try {
          ...
          console.log(
            `[${new Date().toISOString()}] 💼 Balance: ${formatBalance(currentBalance.available)}`
          );
        } catch (error) {
